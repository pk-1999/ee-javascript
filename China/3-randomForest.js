var GMD_GM2020 = ee.FeatureCollection("projects/ee-pkyu1999/assets/mangrove/GlobalMangrove2020"),
    ...
    S2_and_L8 = ee.ImageCollection("projects/ee-pkyu1999/assets/S2_and_L8-China");
...
// RF
var trainedClassifier_RF = ee.Classifier.smileRandomForest(250).train({
...
var trainedClassifier = trainedClassifier_RF;
...
// 可视化分类后处理结果
Map.addLayer(filteredImage, { min: 0, max: 1, palette: ['red', 'blue'] }, 'Filtered Classification', false);
...
Map.addLayer(GMD_LREIS, { color: 'yellow', opacity: 0.3 }, 'GMD_LREIS', false);
//Map.addLayer(GMD_USGS2015, {color: 'yellow', opacity: 0.3},'GMD_USGS2015', false);
//Map.addLayer(GMD_GMW2010, {color: 'yellow', opacity: 0.3},'GMD_GMW2010', false);
...
var videoArgs = {
    ...
    max: 1,
    palette: ['red', 'blue'],
    dimensions: 1080,  // 视频分辨率
    framesPerSecond: 1,  // 每秒帧数
    // region: roi,  // 替换为你感兴趣的区域
};

// 生成视频 URL
var videoUrl = classified.getVideoThumbURL(videoArgs);
// 打印视频 URL
