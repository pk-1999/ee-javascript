var Points_China = ee.FeatureCollection("projects/ee-pkyu1999/assets/PointsWithValue/RMD_GM2020_China_first");
...
// *************
var newfc = Points_China;

function selectSampleValue(image) {
    image = ee.Image(image);
    var training = image.select(bands).sampleRegions({
        collection: newfc,   //采样范围，从newfc中的所有点
        properties: [classProperty],  //保留的属性
        scale: 30,   //采样的空间分辨率
        geometries: true  // 保留地理信息
    });
    return training;
}

var S2_samplePoints_list = S2_imageList.map(selectSampleValue);

for (var i = 0; i < 2; i++) {
    var toAssetPoints = ee.FeatureCollection(S2_samplePoints_list.get(i));
    var f_name = name_image.get(i).getInfo();
    // 检查
    print(toAssetPoints.size());
    // 导出
    Export.table.toDrive({
        collection: toAssetPoints,
        folder: folder,
        fileNamePrefix: 'S2-' + f_name,
        description: 'sample_points-S2-' + f_name,
    });
}
